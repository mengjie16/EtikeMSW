...
$.validator.methods.checkRange = function(value, element, param) {
	...
	if(parseFloat(eValue)>parseFloat($ecompare)) {
		return false;
	}
	return true;
};
...
function initBase(argument) {
	...
	$(document).on('click', '.pdelbtn', function() {
		...
		$(this).parent().remove();
		if (ilen <= 4) $('#addPriceItem').show();
	});
	// 增加收货地址弹框	
	$(document).on('click', '#addAddress', function() {
		// 加载历史记录
		if (!CDT.sendLoactionTempCache) {
			Tr.get('/supplier/sendLocation/list', {}, function(data) {
	...
	$(document).on('click', '#addAddressBlock #btnAddfirm', function() {
		...
		if (checked && saveTemp) {
			Tr.post('/supplier/sendlocation/confirm', loactionTemp, function(data) {
				if (data.code != 200) {
					var msg = '添加失败!';
					if (data.code == 8001) {
						msg = '该地址已在地址模板中存在!';
...
$(function() {
	 //window.onbeforeunload=function(event){  return "确认离开当前页面？您的商品编辑还未保存";};
