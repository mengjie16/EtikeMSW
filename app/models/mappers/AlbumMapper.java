...
public interface AlbumMapper {
    ...
    @Update("update "+Album.TABLE_NAME+" SET album_items = JSON_ARRAY_APPEND(album_items,'$',CAST('${album}' AS JSON)) WHERE id =${id}")
    ...
    @Update("update "+Album.TABLE_NAME+" SET album_items = JSON_SET(album_items,'$[${index}].itemId',${itemId}) WHERE id =${id}")
    int updateAlbumItemIdByIndex(@Param("index") int index,@Param("itemId") long itemId,@Param("id") long id);
    
    @Update("update "+Album.TABLE_NAME+" SET album_items = JSON_SET(album_items,'$[${index}].price',${price}) WHERE id =${id}")
    int updateAlbumItemPriceByIndex(@Param("index") int index,@Param("price") float price,@Param("id") long id);
    
    @Update("update "+Album.TABLE_NAME+" SET album_items = JSON_SET(album_items,'$[${index}].picUrl','${picUrl}') WHERE id =${id}")
