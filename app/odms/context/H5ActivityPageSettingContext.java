 ...
 */
public class H5ActivityPageSettingContext extends MdbContext<H5ActivityPageSetting> {
	
    public H5ActivityPageSettingContext() {
        COLLECTION_NAME = "activity_page_setting_h5";
    }
     ...
     */
    public H5ActivityPageSetting findBySettingById(String _id) {
     ...
     */
    public List<H5ActivityPageSetting> findPageList(int pageNumber, int pageSize) {
        DBObject dbObj = new BasicDBObject();
        ...
        List<H5ActivityPageSetting> activityList = Lists.transform(getObjs, new Function<DBObject, H5ActivityPageSetting>() {
            @Override
            public H5ActivityPageSetting apply(DBObject input) {
     ...
     */
    public H5ActivityPageSetting findCurrentUseSetting(){
     ...
     */
    public boolean save(H5ActivityPageSetting setting) {
     ...
     */
    public String saveReturnId(H5ActivityPageSetting setting) {
        Validate.isTrue(setting != null, "setting can't be null!");
     ...
     */
    private DBObject transform(H5ActivityPageSetting setting) {
        Validate.isTrue(setting != null, "setting can't be null !");
        DBObject dbObj = new BasicDBObject();
     ...
     */
    protected H5ActivityPageSetting parseFromRawObject(DBObject dbObj){
        if(dbObj==null){
            return null;
        }
        H5ActivityPageSetting h5aps = new H5ActivityPageSetting();
        h5aps._id = (String)dbObj.get("_id");
        List<BasicDBList> dbAlist = (List<BasicDBList>)dbObj.get("groupItems");
        if(MixHelper.isNotEmpty(dbAlist)){
            h5aps.groupItems = Lists.transform(dbAlist, new Function<BasicDBList, Long[]>(){
                @Override
                public Long[] apply(BasicDBList input) {
                    if(MixHelper.isNotEmpty(input)){
                        Long[] ids = new Long[input.size()];
                        input.toArray(ids);
                        return ids;
                    }
                    return null;
        ...
        h5aps.title = (String)dbObj.get("title");
        h5aps.isUse = (Boolean)dbObj.get("isUse");
        h5aps.createTime = (String)dbObj.get("createTime");
        return h5aps;
    }
